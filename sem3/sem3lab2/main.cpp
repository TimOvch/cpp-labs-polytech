...
int main()
{
	...
	// Объясните назначение условия if (this == &v). Чтобы не выполнять лишних действий, если мы приравниваем объект к нему же
...
//    }
	...
	Vector c (a); // Поскольку при инициализации во время определения вызывается конструктор копирования
...
//  Обычно операции *,/,+.- определяются внешней функцией, для того чтобы можно было реализовать операции со встроенными типами,
//  но в этом случае в операции используются 2 пользовательских типа и поэтому логичнее всего реализовать методом класса
